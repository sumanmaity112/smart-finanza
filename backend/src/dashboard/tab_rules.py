...
def render_rules_tab(tracker, run_query):
    ...
    st.markdown("Define rules to automatically categorize transactions.\n\n"
    ...
    # --- Fetch Context ---
    try:
        cat_df, _ = run_query("SELECT DISTINCT category FROM transactions WHERE category IS NOT NULL AND category != '' ORDER BY category")
    ...
    if "cat_suggestion_clicked" in st.session_state and st.session_state.cat_suggestion_clicked:
    ...
    with st.container(border=True):
        ...
        with col2:
            ...
            if new_category:
                suggestions = [cat for cat in existing_categories if new_category.lower() in cat.lower()]
                if suggestions:
        ...
        if new_keyword:
            ...
            if not preview_df.empty:
                st.info(f"🔎 **Preview:** This rule will match **{len(preview_df)}** visible transactions (likely many more)")
                with st.expander("View matching transactions", expanded=True):
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "new_category": st.column_config.Column("New Category", help="Target Category")
                        },
                        width="stretch",
                        hide_index=True
                    )
            else:
                st.caption("ℹ️ No matches found in current data, but this rule will apply to future uploads.")
        # --- SAVE BUTTONS ---
            ...
            if st.button("Save & Apply Rule", type="primary"):
                if not new_keyword or not new_category:
                    ...
                    try:
                        ...
                        st.toast(f"✅ Rule saved! Future imports will now auto-tag '{new_keyword}'.")
                        reset_add_rule_form()
                        st.rerun()
    ...
    try:
        ...
        rules_df, _ = run_query("SELECT keyword, category FROM category_map ORDER BY keyword, category")
    except Exception:
        rules_df = pd.DataFrame()
    if not rules_df.empty:
        search = st.text_input("🔍 Search existing rules...", placeholder="Filter by merchant or category...")
        if search:
            mask = (
                rules_df['keyword'].str.contains(search, case=False) |
        ...
        with r_col1:
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "keyword": st.column_config.TextColumn("Merchant Keyword", width="large"),
        ...
        with r_col2:
            ...
            if available_rules:
                rule_to_delete = st.selectbox("Select rule to remove", available_rules, key="del_select")
                if st.button("Delete Selected", type="secondary"):
                    if st.confirm(f"Are you sure you want to delete the rule for '{rule_to_delete}'?"):
                        try:
                            conn = tracker.db.get_connection()
                            conn.execute("DELETE FROM category_map WHERE keyword = ?", (rule_to_delete,))
                            conn.commit()
                            conn.close()
